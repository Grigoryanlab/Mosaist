...
class fasstCache {
  public:
    ...
    mstreal maxNumFactor() const { return log(sf*maxNumPressure/maxNumResults + 1.0) + 1.0; }
    mstreal errTolFactor() const { return log(sf*errTolPressure/maxNumResults + 1.0) + 1.0; }
  ...
  private:
    class fasstCachedResult {
      ...
      public:
        fasstCachedResult() { solSet = NULL; priority = 1.0; }
        ...
        mstreal getRMSDCutoff() const { return solSet->rbegin()->getRMSD(); }
    ...
    RMSDCalculator rc;
    mstreal errTolPressure, maxNumPressure;
    int sf;
};
