...
class TERMUtils {
  public:
     ...
     * residue(s) belong to. However, by default, non-contiguous residues are
     * not added when the +/- pm extension is performed. This behavior can be
     * changed by specifying the last boolean argument as false.
     * NOTE: the following functions are meant to extract a sensible structure.
     * So, if segments overlap, residues will only be listed once (and the over-
     * lapping segments will be part of a single chain). For this reason, the
     * order in which central residues are specified does not influence the order
     * of residues in the selected TERM, and the latter is dictated by the topo-
     * logy of the structure from which the TERM is excised. But this may not
     * always be what one wants, so look at the function below. */

    /* case 1: single central residue; returns index of the central residue in
     * the final motif structure. */
    static int selectTERM(Residue& cenRes, ConFind& C, Structure& frag, int pm = 2, mstreal cdCut = 0.01, vector<int>* fragResIdx = NULL, bool contiguous = true);
    ...
    static vector<int> selectTERM(const vector<Residue*>& cenRes, ConFind& C, Structure& frag, int pm = 2, mstreal cdCut = 0.01, vector<int>* fragResIdx = NULL, bool contiguous = true);
    /* case 3: same as case 1, but returns a Structure rather than accepting a reference */
    static Structure selectTERM(Residue& cenRes, ConFind& C, int pm = 2, mstreal cdCut = 0.01, vector<int>* fragResIdx = NULL, bool contiguous = true);
    /* case 4: same as case 2, but returns a Structure */
    static Structure selectTERM(const vector<Residue*>& cenRes, ConFind& C, int pm = 2, mstreal cdCut = 0.01, vector<int>* fragResIdx = NULL, bool contiguous = true);
