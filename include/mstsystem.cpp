...
void MstSys::cmkdir(const string& dirPath, bool makeParents) {
  int ret = MstSys::csystem("mkdir " + (makeParents ? "-p " : "") + dirPath, false);
  MstTools::assert(ret == 0, "failed to make directory '" + dirPath + "'");
}

void MstSys::crmdir(const string& dirPath, bool recursive) {
  int ret = MstSys::csystem((recursive ? "rmdir " : "rm -r ") + dirPath, false);
  MstTools::assert(ret == 0, "failed to remove directory '" + dirPath + "'" + (recursive ? " recursively" : ""));
}

void MstSys::crm(const string& filePath) {
  int ret = MstSys::csystem("rm " + filePath, false);
  MstTools::assert(ret == 0, "failed to remove file '" + filePath + "'");
}
